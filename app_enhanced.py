...
def load_yolo_model():
    try:
        ...
        model = YOLO(
            "yolov8n.pt"
        )  # Using nano version for speed, can use larger models for better accuracy
        return model
...
def main():
    ...
    if os.path.exists(model_dir):
        ...
        if model_files:
            selected_model = st.sidebar.selectbox(
                "Select Classification Model", model_files, index=0
            )
            ...
            if not os.path.exists(class_names_path):
                ...
                class_files = [
                    f for f in os.listdir(model_dir) if f.endswith("_classes.json")
                ]
                if class_files:
            ...
            if st.button("🔍 Analyze Image", type="primary", use_container_width=True):
                with st.spinner("Analyzing image..."):
                    ...
                    detections = detect_food_items(
                        image, yolo_model, confidence_threshold=confidence_threshold
                    )

                    # Classify the main image
                    classification = predict_food(
                        image, model, class_names, preprocessor, top_k=top_k
                    )
                    ...
                    if detections:
                        ...
                        with st.expander("🔍 Detection Results", expanded=True):
                            ...
                            if class_counts:
                                ...
                                st.info(
                                    "No food items detected. Try adjusting the confidence threshold."
                                )
                    else:
                        st.warning(
                            "No food items detected. Try adjusting the confidence threshold."
    ...
    with col2:
        ...
        if "classification" in st.session_state:
            ...
            st.metric("Confidence", f"{classification['top_confidence']*100:.2f}%")
            ...
            if "detections" in st.session_state and st.session_state["detections"]:
                ...
                for i, det in enumerate(st.session_state["detections"], 1):
                    with st.expander(
                        f"Detection {i}: {det['class_name']} ({det['confidence']:.2f})"
                    ):
                        st.json(
                            {
